...
jobOffersRouter
  .route('/')
  .get(getJobOffers)
  .post(authenticate, hasRole('self', 'admin'), validateZod(userInputSchema, 'body'), createJobOffer);
jobOffersRouter.use('/:id', validateZod(paramSchema, 'params'));
jobOffersRouter
  .route('/:id')
  .get(getSingleJobOffer)
  .put(authenticate, hasRole('self', 'admin'), validateZod(userInputSchema, 'body'), updateJobOffer)
