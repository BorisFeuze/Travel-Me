...
import { updateJobOffers, getJobOffers } from "@/data/jobOffers"; // getJobById musst du implementieren
...
const EditJob = () => {
  ...
  // Job aus der API oder lokalem Speicher laden
  useEffect(() => {
    if (!id) return;

    const fetchJob = async () => {
      try {
        const job: JobFormData = await getJobOffers(id); // implementiere getJobById
        setFormData({
          ...job,
          userProfileId: user?._id || job.userProfileId,
        });

        // Preview URLs aus vorhandenen Bildern setzen
        const urls = job.pictureURL.map((p) => (typeof p === "string" ? p : ""));
        setPreviewUrls(urls);
      } catch (err) {
  ...
  if (!formData) return <div>Lade Job...</div>;
  ...
  const nextImage = () =>
    setCurrentIndex((prev) => (previewUrls.length > 1 ? (prev + 1) % previewUrls.length : prev));
  const prevImage = () =>
    setCurrentIndex((prev) =>
      previewUrls.length > 1 ? (prev === 0 ? previewUrls.length - 1 : prev - 1) : prev
    );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateJobOffers(data);

      setSaveMessage({ text: "Job offer updated!", type: "success" });
    } catch (err) {
      console.error(err);
      setSaveMessage({ text: "Error while updating job offer.", type: "error" });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-linear-to-b from-blue-50 to-purple-50 p-6 flex flex-col lg:flex-row gap-8">
      ...
      <div className="w-full lg:w-2/3">
        <div className="card bg-white shadow-2xl rounded-2xl p-6 pb-3" style={{ minHeight: "600px" }}>
          ...
          <input type="text" className="input input-bordered w-full mb-4 shadow-sm focus:ring-2 focus:ring-gray-400 transition"
            value={formData.continent} onChange={(e) => handleInputChange("continent", e.target.value)} />
        ...
        {/* location */}
          ...
          <Calendar02
            ...
            onSelectDates={(dates) => handleInputChange("availability", dates)}
          />
